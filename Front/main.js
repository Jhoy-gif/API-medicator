    ...
    if(nome.checked){
        if(input == "") {
            ...
            try {
                ...
                let response = await fetch("http://localhost:8080/medicine/get_name?name=" + input).then(body => body.json());
        ...
        if(input == "") {
            ...
            try {
                // mudar rota de acordo com o arquivo que tiver testando
                let response = await fetch("http://localhost:8080/medicine/get_eans?ean=" + input).then(body => body.json());
                createTableFromJSON(response);
                id = 1
            } catch (error) {
                console.log(error) /* Tratar error */
        ...
        if(input == "") {
            ...
            try {
                // mudar rota de acordo com o arquivo que tiver testando
                let response = await fetch("http://localhost:8080/medicine/get_active?subs=" + input).then(body => body.json());
                createTableFromJSON(response);
                id = 1
            } catch (error) {
                console.log(error) /* Tratar error */
        ...
        if(input == "") {
            ...
            try {
                // mudar rota de acordo com o arquivo que tiver testando
                let response = await fetch("http://localhost:8080/medicine/get_ggrem?ggrem=" + input).then(body => body.json());
                createTableFromJSON(response);
                id = 1
            } catch (error) {
                console.log(error) /* Tratar error */
    ...
    for (var i = 0; i < response.length; i++) {
        ...
        for (var j = 0; j < col.length; j++) {
           if(j == 0 || j == 2 || j == 8 || j == 9 || j == 11 || j == 12 || j == 13 || j == 32 || j == 39) {
                ...
                tabCell.innerHTML = response[i][col[j]].replace(/;/gi, " ");
           }
        }
    }
